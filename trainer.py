    ...
    def _get_reference_text_image_features(self, labels, image_features):
        ...
        if len(image_features.size()) == 3:
            for i in range(image_features.shape[1]):
                ref_img_features.append(utils.norm(image_features[:, i, :]).half())
            #image_features = image_features[:, 0, :]
        #ref_img_features = utils.norm(image_features).half()
        return ref_text, ref_text_features, torch.stack(ref_img_features)
...
class ContrastiveTrainer(SelfTrainerWithClip):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.state.epoch < self.additional_args.start_contrastive_at_epoch:
            ...
            text_img_info_nce = []
            for i in range(ref_img_features.shape[1]):
                text_img_info_nce.append(self.contrastive_loss(query=hyp_text_features, positive_key=ref_img_features[i,:,:]))
            
            ## get the average of  
            text_img_info_nce = torch.mean(torch.stack(text_img_info_nce))
